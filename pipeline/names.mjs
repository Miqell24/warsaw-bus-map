...
export const RULES = { lowerAbbr: true,
  // the town-and-stop feeds repeat the name when the stop is named after its
  // village ("Zawady Zawady"); three R9 stops carry a "Warszawa " the ZTM
  // names of the same poles do not (the railway stations keep theirs: that is
  // the station's name)
  stopRe: [[/^(\p{L}+) \1(?=\s|$)/u, '$1']],
  stops: { 'Warszawa Metro Kondratowicza': 'Metro Kondratowicza', 'Warszawa CH Targówek': 'CH Targówek', 'Warszawa Geodezyjna': 'Geodezyjna' },
  keys: { '83 EXPRESS CHEŁMOŃSKI': '83' },
};
